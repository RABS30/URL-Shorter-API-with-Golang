...
func Test_ForgotPasswordHandler_Pass(t *testing.T) {
	...
	wrapper := &middleware.ResponseWriterWrapper{ResponseWriter: rec, StatusCode: http.StatusOK, ErrorLog: "-"}

	mockService.On("RequestResetPassword", mock.Anything, "rabs@example.com").Return(nil)

	h := NewPasswordResetTokensHandler(mockService)
	h.ForgotPasswordHandler(wrapper, req, httprouter.Params{})

	assert.Equal(t, http.StatusOK, rec.Code)
	assert.Contains(t, rec.Body.String(), "success")
	assert.Equal(t, "-", wrapper.ErrorLog)
...
func Test_ForgotPasswordHandler_Fail_InvalidPayload(t *testing.T) {
	...
	assert.NotEqual(t, "-", wrapper.ErrorLog)
	assert.NotEmpty(t, wrapper.ErrorLog)
}
...
func Test_ResetPasswordHandler_Pass(t *testing.T) {
	...
	h := NewPasswordResetTokensHandler(mockService)
	h.ResetPasswordHandler(wrapper, req, httprouter.Params{})
...
func Test_ResetPasswordHandler_Fail_ServiceValidationError(t *testing.T) {
	...
	mockService.On("ExecuteResetPassword", mock.Anything, "valid-token-rabs", "NewPass123", "NewPass123").Return(errors.New("password not equal"))
